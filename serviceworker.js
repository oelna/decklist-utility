...
(function() {
	...
	var sitePath = '/~oelna/decklist-utility';
	var version = 'v1::';
	...
	function updateStaticCache() {
		return caches.open(version + staticCacheName)
			.then(function (cache) {
				return cache.addAll([
					sitePath+'/unpack.js',
					sitePath+'/deckcode_lib.js',
					sitePath+'/deckcode_frontend.js',
					sitePath+'/deckcode.html',
	...
	self.addEventListener('fetch', function (event) {
		...
		if (request.method !== 'GET') {
			event.respondWith(
				fetch(request)
					.catch(function () {
						return caches.match(sitePath+'/offline.html');
					})
			);
			return;
		}
